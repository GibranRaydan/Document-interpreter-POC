...
async def extract_text(file_path: str, client: httpx.AsyncClient, agent: Agent) -> OCRResult:
    ...
    messages = [{"role": "user", "content": agent.system_prompt, "images": images_b64}]
    response: LLMResponse = await call_llm(client, agent.model, messages)
    return OCRResult(raw_text=response.content, tokens_input=response.tokens_input, tokens_output=response.tokens_output)
